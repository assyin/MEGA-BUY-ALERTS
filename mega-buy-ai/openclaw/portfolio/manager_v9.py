"""Portfolio Manager V9 — V7 Hybrid Trailing + Ultra Filter (ADX 15-35, BTC bull, 24h>=1%).

Based on V7 (Hybrid Trailing TP) with 3 additional rules:
- R1: ADX 4H between 15 and 35
- R2: BTC 1H must be BULLISH
- R3: 24h change >= 1%

Simulation on V7 trades: 90% WR (9W/1L) vs 56% baseline. 0 winners rejected.
"""
...
class PortfolioManagerV9:
    ...
    TABLE = "openclaw_positions_v9"
    STATE_TABLE = "openclaw_portfolio_state_v9"
    ...
    async def try_open_position(self, pair: str, decision: str, confidence: float,
        ...
        # V9 BASE GATE — DI+ max relaxed to 65 if vol bypass
        if vol_bypass:
            passed, reason = passes_optimized_gate(pair, alert, label="V9+Vol", cache=alert.get("_gate_cache"))
            ...
            if not passed:
                print(f"💼 V9 GATE REJECT {pair}: {reason}")
                return None
        else:
            passed, reason = passes_optimized_gate(pair, alert, label="V9", cache=alert.get("_gate_cache"))
            if not passed:
                print(f"💼 V9 GATE REJECT {pair}: {reason}")
                return None
        ...
        if di_spread >= 50:
            print(f"💼 V9 REJECT {pair}: D±={di_spread:+.0f} >= 50 (movement exhausted)")
            return None

        # V9 ULTRA FILTER: ADX 15-35 (or 15-40 with vol bypass), BTC bull, 24h >= 1%
        ...
        try:
            ...
            if _kd and isinstance(_kd, list) and len(_kd) > 0:
                ...
                if btc_c < btc_o:
                    print(f"💼 V9 REJECT {pair}: BTC 1H bearish")
                    return None
        except:
            pass
        ...
        if stc_15m is not None and stc_15m >= 0.99:
            print(f"💼 V9 REJECT {pair}: STC15m={stc_15m:.2f} >= 0.99")
            return None
        # STC30 filter removed — high STC30 correlates with big winners
        if stc_1h is not None and stc_1h < 0.1:
            print(f"💼 V9 REJECT {pair}: STC1h={stc_1h:.3f} < 0.1 (1H bearish)")
            return None

        # Volume filter: reject if ALL 4 vol spikes are negative
        ...
        if all(v is not None and v < 0 for v in _fp_vol.values()):
            print(f"💼 V9 REJECT {pair}: all 4 vol spikes negative — no buying pressure")
            return None
        ...
        if len(open_positions) >= self.MAX_POSITIONS:
            print(f"💼 V9 SKIP {pair}: portfolio full ({self.MAX_POSITIONS}/{self.MAX_POSITIONS})")
            return None
        ...
        try:
            ...
            print(f"⚠️ V9 insert: {e}")
            return None

        self._update_state({"balance": round(balance - size_usd, 2)})
        print(f"💼 V9 OPENED: {pair} — {confidence*100:.0f}% — ${size_usd:.0f} @ {price} | TP1={tp1} TP2={tp2} SL={sl}")
    ...
    async def check_positions(self):
        ...
        for pos in positions:
            ...
            if not partial1 and tp1 and price >= tp1:
                ...
                print(f"💼 V9 TP1 ✅: {pos['pair']} closed 50% @ +{self.TP1_PCT}% (${profit_usd:+.1f}) — SL→BE")
            ...
            if partial1 and not partial2 and tp2 and price >= tp2:
                ...
                print(f"💼 V9 TP2 ✅✅: {pos['pair']} closed 30% @ +{self.TP2_PCT}% (${profit_usd:+.1f}) — Trailing 8% activé sur 20% restants")
        ...
        if checked or closed:
            print(f"💼 V9 check: {checked} updated, {closed} closed ({len(positions)} open)")
    ...
    async def _close_full(self, pos: Dict, exit_price: float, reason: str,
        ...
        print(f"💼 V9 {'✅' if is_win else '❌'}: {pos['pair']} {pnl_pct_position:+.1f}% (${total_realized_pnl:+.1f}) — {reason}")
        ...
        await self._tg(
            f"{emoji} *V9 CLOSE FULL* — `{pos['pair']}` — *{reason}*\n"
    ...
    async def start(self):
        ...
        print(f"💼 V9 Portfolio started — Body≥3% + Hybrid Trailing | 12 slots × 8% × $5000 | TP1=50%@+10% TP2=30%@+20% Trail=8%")
